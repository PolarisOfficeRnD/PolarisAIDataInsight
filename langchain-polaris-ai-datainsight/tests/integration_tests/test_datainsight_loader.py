...
def test_lazy_load__load_with_element_mode(temp_resources_dir, mock_response):
...
@pytest.mark.usefixtures("temp_resources_dir")
@pytest.mark.usefixtures("mock_response")
def test_lazy_load__load_with_page_mode(temp_resources_dir, mock_response):
    loader = PolarisAIDataInsightLoader(
        file_path=EXAMPLE_DOC_PATH,
        api_key="api_key",
        resources_dir=temp_resources_dir,
    ...
    for page_id, doc in enumerate(docs):
        ...
        assert len(doc.metadata.get("elements")) == MOCK_RESPONSE_DATA_STRUCTURE["pages"][str(page_id + 1)]["total"]
        assert len(doc.metadata.get("resources")) == MOCK_RESPONSE_DATA_STRUCTURE["pages"][str(page_id + 1)]["image"]
        ...
        assert len(resource_ids) == MOCK_RESPONSE_DATA_STRUCTURE["pages"][str(page_id + 1)]["image"]
...
@pytest.mark.usefixtures("temp_resources_dir")
@pytest.mark.usefixtures("mock_response")
def test_lazy_load__load_with_single_mode(temp_resources_dir, mock_response):
    loader = PolarisAIDataInsightLoader(
        file_path=EXAMPLE_DOC_PATH,
        api_key="api_key",
        resources_dir=temp_resources_dir,
