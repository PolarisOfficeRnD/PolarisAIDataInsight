...
SupportedExtensionType = Literal['.doc', '.docx', '.ppt', '.pptx', '.xls', '.xlsx', '.hwp', '.hwpx', '.pdf']
StrPath = str | Path
...
class PolarisAIDataInsightExtractor:
    ...
    def __init__(self, *args, **kwargs):
        ...
        # Set base url
        self._api_base_url = os.environ.get("DATA_INSIGHT_BASE_URL", "https://datainsight-api.polarisoffice.com/api/v1/datainsight/doc-extract")
        self._supported_extensions = get_args(SupportedExtensionType)
        self.blob: Blob = None
        self.resources_dir: StrPath = kwargs.get("resources_dir", "app/")
        self.api_key: str = kwargs.get("api_key", os.environ.get("POLARIS_AI_DATA_INSIGHT_API_KEY"))
    ...
    def _get_response(self, blob: Blob) -> requests.Response:
        try:
            ...
            files = {
                "file": (filename, blob.data, blob.mimetype)
            }
